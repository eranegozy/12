...
class SimpleTempoMap(object):
   ...
   # set a new tempo so that we have tick continuity 
   # at the given time
...
class AudioScheduler(object):
   ...
   def generate(self, num_frames, num_channels) :
      ...
      # print self.cur_frame, end_frame, self.commands
      ...
      while self.commands:
         ...
         # print '   ', cmd_tick, cmd_time, cmd_frame
...
class AudioClock(object):
   def __init__(self):
      super(AudioClock, self).__init__()
      self.generator = None
      self.frame_cnt = 0
   ...
   def generate(self, num_frames, num_channels) :
      if self.generator:
         ...
         data, cont = np.zeros(num_channels * num_frames, dtype=np.float32), True
      self.frame_cnt += num_frames
      return data, cont

   def get_time(self) :
      return self.frame_cnt / float(kSampleRate)
